...
const AnalyticsModule = {
  ...
  renderStats() {
    ...
    document.getElementById('ana-tasks-30').textContent = tasks30.length;
    document.getElementById('ana-done-30').textContent = done30.length;
    document.getElementById('ana-velocity').textContent = velocity;
    document.getElementById('ana-journal-30').textContent = journal30.length;
  },
    ...
    if (ctx1) {
      ...
      this.charts.tasks = new Chart(ctx1, {
        ...
        data: {
          ...
          datasets: [{
            data: [counts.todo, counts['in-progress'], counts.review, counts.done],
  ...
  renderStorage() {
    ...
    el.innerHTML = Object.entries(stats.modules)
      ...
      .map(([key, info]) => `
        <div style="display:flex;justify-content:space-between;padding:8px 12px;border-bottom:1px solid var(--divider);">
          <span class="text-sm">${Utils.escape(key)}</span>
          <div style="display:flex;gap:16px;">
            <span class="text-sm text-muted">${info.items} items</span>
            <strong class="text-sm">${info.size}</strong>
          </div>
        </div>
